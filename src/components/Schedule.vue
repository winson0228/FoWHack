    ...
    <div>
         <md-card class="analysis"> 
            ...
            <div class="pad">
                ...
                <md-progress-bar class="md-primary" md-mode="determinate" :md-value="results.effectivePercent"></md-progress-bar>
            </div>
            <div class="pad">
                {{ wastedMinsMessage }} of Interrupted Work
                <md-progress-bar class="md-accent" md-mode="determinate" :md-value="results.wastedPercent"></md-progress-bar>
            </div>
            <div class="pad">      
...
export default {
  ...
  components: {
   ...
   computed: {
       wastedMinsMessage() {
        ...
        effectiveMinsMessage() {
           return Moment.duration(this.results.effectiveMins, 'minutes').humanize()
       },
        meetingMinsMessage() {
   ...
   mounted () {
    ...
    //        this.events = val;
    //    });
        store.watch(() => store.getters.events, (val) => {
            console.log(val);
            this.events = val;
       });
       this.displayResults();
       ...
       calculateEffectiveHours(events) {
            ...
            for(let key of Object.keys(bins)) {
                bins[key] = bins[key].sort((a, b) => {
                    return new Moment(a.start).format('X') - new Moment(b.start).format('X')
                });

                let endTime = null;
                for(let event of bins[key]) {
                    if(endTime) {
                        results.meetingMins += Moment.duration(new Moment(event.end).diff(new Moment(event.start))).asMinutes();
                        let workMins = Moment.duration(new Moment(event.start).diff(endTime)).asMinutes()
                        if(workMins > 60) {
            ...
            let total = results.meetingMins + results.wastedMins + results.effectiveMins;
